...
export function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const { login, isLoggedIn, username: loggedInUsername } = useAuth()
  ...
  return (
    ...
    >
      <div className="py-6 px-2">
        {/* Avatar - Square, no ring, smaller */}
        <div className="flex justify-center mb-3">
          <div className="w-14 h-14 rounded-md overflow-hidden bg-white/5">
            <img
              src={avatarUrl}
              alt={isLoggedIn ? loggedInUsername || 'User' : 'Login'}
        ...
        {/* Edition Selection */}
        <div className="mb-4">
          <p className="text-text-muted text-base text-center mb-3">
            Select your Minecraft edition
          </p>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => setEdition('java')}
              className={`flex items-center justify-center gap-2 px-4 py-3 rounded-sm transition-all duration-200 ${
                edition === 'java'
            ...
            >
              ...
              <span className="font-medium text-sm">Java Edition</span>
            </button>
            <button
            ...
            >
              ...
              <span className="font-medium text-sm">Bedrock Edition</span>
            </button>
          </div>
        </div>
