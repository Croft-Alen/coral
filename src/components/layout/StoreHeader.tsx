...
export function StoreHeader() {
  ...
  const avatarUrl = isLoggedIn && username
    ? `https://minotar.net/avatar/${encodeURIComponent(username)}/64`
    : DEFAULT_AVATAR
  ...
  const showStaffLink = settings?.showStaffLink !== false
  const showVoteLink = settings?.showVoteLink !== false
  const showRulesLink = settings?.showRulesLink !== false

  const hasNavLinks = showStaffLink || showVoteLink || showRulesLink
